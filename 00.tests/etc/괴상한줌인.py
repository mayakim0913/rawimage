...
#######
    ...
    def YUYV_clicked(self):
       ...
       # pixmap = pixmap.scaled(self.imgwidth,self.imgheight,QtCore.Qt.KeepAspectRatio)
...
#######
	...
	self.label_img.setPixmap(picture)
    ...
    def scaleImage(self, factor):
        ...
        self.label_img.resize(self.scaleFactor * self.label_img.pixmap().size())
...
'''
        ...
        self.label_img.setSizePolicy(QtWidgets.QSizePolicy.Ignored,
...
#	self.scaleImage(1.25)
	...
	self.imgwidth = self.imgwidth * 1.2
	self.imgheight = self.imgheight * 1.2
	self.update()
...
'''
        ...
        sub.show()
	self.SetYUVchk()
        sub.resize(self.imgwidth, self.imgheight)
       # sub.resize(500,500)
...
class MainWindow(QMainWindow):
    ...
    def zoomIn(self, sub):
        self.scaleImage(sub, 1.2)

    def zoomOut(self):
        self.scaleImage(0.8)

    def scaleImage(self, sub, factor):
        self.scaleFactor *= factor
	self.imgwidth = self.scaleFactor * self.imgwidth
	self.imgheight = self.scaleFactor * self.imgheight
	self.update()

#        self.sub.resize(self.scaleFactor * self.sub.pixmap().size())
